...
def _create_example_for_sent_ace(instance:Dict, max_word_len: int=200) -> List[InputExample]:
    ...
    sent_exs = []
    for event in instance['candidate_events']:
        uid = event['id'] 
        label = -1 # placeholder 
...
def _convert_example_to_mask_feature(ex: InputExample, tokenizer: PreTrainedTokenizer, prompt_idx: int =0) -> Dict: 
    ...
    if prompt_idx == 0:
        ...
        mask_word_prefix = input_tokens + trigger_tokens  + ['is', 'the']
    else:
...
class E2EEventDataModule(pl.LightningDataModule):
    ...
    def _split_types_ace(dataset_dir: str, known_types:int) -> Tuple[List[InputExample], List[InputExample]]:
        file_name = 'candidate_events.json'
        example_by_name = defaultdict(list)
        with open(os.path.join(dataset_dir, file_name)) as f:
            for line in f:
        ...
        sorted_names = sorted_names[1:] + ["unknown"] 
...
if __name__ == '__main__':
    ...
    data_module = E2EEventDataModule(args, tokenizer, args.dataset_dir)
